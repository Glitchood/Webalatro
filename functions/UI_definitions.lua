...
function create_UIBox_notify_alert(_achievement, _type)
  ...
  local _atlas =
    ...
    _type == 'Back' and G.ASSET_ATLAS[(_c and _c.atlas) or "centers"] or
    G.ASSET_ATLAS["icons"]
...
function create_UIBox_online_high_scores()
  if G.HTTP_MANAGER and G.HTTP_MANAGER.out_channel then
...
function G.UIDEF.shop()
  ...
  function create_card_for_shop(area)
      if area == G.shop_jokers and (not G.SETTINGS.tutorial_complete) and G.SETTINGS.tutorial_progress and G.SETTINGS.tutorial_progress.forced_shop and G.SETTINGS.tutorial_progress.forced_shop[#G.SETTINGS.tutorial_progress.forced_shop] then
...
function create_UIBox_HUD()
    ...
    contents.buttons = {
      {n=G.UIT.C, config={align = "cm", r=0.1, colour = G.C.CLEAR, shadow = true, id = 'button_area', padding = 0.2}, nodes={
          ...
          -- Skip Tutorial button (only show when in tutorial mode)
          (G.GAME and G.GAME.pseudorandom and G.GAME.pseudorandom.seed == "TUTORIAL") and
          {n=G.UIT.R, config={align = "cm", minh = 1.75, minw = 1.5,padding = 0.05, r = 0.1, hover = true, colour = G.C.GREEN, button = "skip_tutorial", shadow = true}, nodes={
            {n=G.UIT.C, config={align = "cm", maxw = 1.4}, nodes={
              {n=G.UIT.T, config={text = "Skip", scale = scale, colour = G.C.UI.TEXT_LIGHT, shadow = true}}
            }},
            {n=G.UIT.C, config={align = "cm", maxw = 1.4}, nodes={
              {n=G.UIT.T, config={text = "Tutorial", scale = scale*0.8, colour = G.C.UI.TEXT_LIGHT, shadow = true}}
            }},
          }} or nil
        }}
    }
...
function create_UIBox_options()  
  ...
  local save_hint = {n=G.UIT.R, config={align = "cm", padding = 0.05}, nodes={
    {n=G.UIT.T, config={text = "Import/Export saves in Profile", scale = 0.35, colour = G.C.UI.TEXT_INACTIVE}}
  }}
...
function G.UIDEF.settings_tab(tab)
  if tab == 'Game' then
    return {n=G.UIT.ROOT, config={align = "cm", padding = 0.05, colour = G.C.CLEAR}, nodes={
      create_option_cycle({label = localize('b_set_gamespeed'),scale = 0.8, options = {0.25, 0.5, 1, 2, 3, 4, 8, 16}, opt_callback = 'change_gamespeed', current_option = (
      ...
      G.IS_WEB and create_slider({label = "Mouse Sensitivity", w = 4, h = 0.4, ref_table = G.SETTINGS, ref_value = 'mouse_sensitivity', min = 10, max = 100}) or nil,
  ...
  elseif tab == 'Graphics' then 
    return {n=G.UIT.ROOT, config={align = "cm", padding = 0.05, colour = G.C.CLEAR}, nodes={
      ...
      create_toggle({label = "Show FPS Counter", ref_table = G.SETTINGS, ref_value = 'show_usage'}),
    }}
  end
...
function create_UIBox_your_collection_tarots()
  ...
  local tarot_options = {}
  local tarot_pages = math.max(1, math.ceil(#G.P_CENTER_POOLS.Tarot/11))
  for i = 1, tarot_pages do
    table.insert(tarot_options, localize('k_page')..' '..tostring(i)..'/'..tostring(tarot_pages))
  end
...
function create_UIBox_your_collection_spectrals()
  ...
  local spectral_pages = math.max(1, math.ceil(#G.P_CENTER_POOLS.Spectral/9))
  for i = 1, spectral_pages do
    table.insert(spectral_options, localize('k_page')..' '..tostring(i)..'/'..tostring(spectral_pages))
  end
...
function create_UIBox_your_collection_decks()
  ...
  for k, v in ipairs(G.P_CENTER_POOLS.Back) do
    ...
    ordered_keys[#ordered_keys+1] = v.key
  end
  
  local t = create_UIBox_generic_options({ back_func = 'your_collection', contents = {
    create_option_cycle({options = ordered_names, deck_keys = ordered_keys, opt_callback = 'change_viewed_back', current_option = 1, colour = G.C.RED, w = 4.5, focus_args = {snap_to = true}, mid = 
              ...
              {n=G.UIT.R, config={align = "cm", padding = 0.2, colour = G.C.BLACK, r = 0.2}, nodes={
                ...
                {n=G.UIT.C, config={align = "tm", minw = 3.7, minh = 2.1, r = 0.1, colour = G.C.L_BLACK, padding = 0.1}, nodes={
                  ...
                  {n=G.UIT.R, config={align = "cm", colour = G.C.WHITE, emboss = 0.1, minh = 2.2, r = 0.1}, nodes={
                    {n=G.UIT.O, config={id = G.GAME.viewed_back.effect.center.key, func = 'RUN_SETUP_check_back', object = UIBox{definition = G.GAME.viewed_back:generate_UI(), config = {offset = {x=0,y=0}}}}}
                  }}       
                }},
              }},
            }}}),
...
function G.UIDEF.run_setup(from_game_over)
  ...
  local _can_continue = G.FUNCS.can_continue({config = {func = true}})
...
function G.UIDEF.profile_option(_profile)
  ...
  local prev_name = G.PROFILES[_profile].name
  ...
  local new_name = profile_data and profile_data.name or (prev_name and prev_name ~= '' and prev_name or ("P".._profile))
  local source = profile_data and 'file' or 'missing'
  if prev_name ~= new_name then
    print(string.format("[PROFILE_OPTION] profile %d name change %s -> %s (%s)", _profile, tostring(prev_name), tostring(new_name), source))
  end
  G.PROFILES[_profile].name = new_name
  ...
  local t = {n=G.UIT.ROOT, config={align = 'cm', colour = G.C.CLEAR}, nodes={
    ...
    {n=G.UIT.R, config={align = "cm", padding = 0.1}, nodes={
      ...
      {n=G.UIT.C, config={align = "cm", minh = 4}, nodes={
        ...
        {n=G.UIT.R, config={align = "cm", padding = 0.1}, nodes={
          {n=G.UIT.R, config={align = "cm", minw = 3, maxw = 4, minh = 0.6, padding = 0.2, r = 0.1, hover = true, colour = G.C.BLUE, button = "export_save", shadow = true, focus_args = {nav = 'wide'}}, nodes={
            {n=G.UIT.T, config={text = "Export Save", scale = 0.3, colour = G.C.UI.TEXT_LIGHT}}
          }},
          {n=G.UIT.R, config={align = "cm", minw = 3, maxw = 4, minh = 0.6, padding = 0.2, r = 0.1, hover = true, colour = G.C.ORANGE, button = "import_save", shadow = true, focus_args = {nav = 'wide'}}, nodes={
            {n=G.UIT.T, config={text = "Import Save", scale = 0.3, colour = G.C.UI.TEXT_LIGHT}}
          }},
        }},
    }},
    }},
...
function G.UIDEF.run_setup_option(type)
  if not G.SAVED_GAME then
    ...
    if raw ~= nil then
      ...
      if ok_unpack then G.SAVED_GAME = unpacked end
    end
  end

  G.SETTINGS.current_setup = type
  local memory = G.PROFILES[G.SETTINGS.profile].MEMORY or {}
  local memory_deck = memory.deck_key or memory.deck
  G.GAME.viewed_back = Back(get_deck_from_name(memory_deck))
  ...
  if type == 'Continue' then 
    ...
    if G.SAVED_GAME ~= nil then
      ...
      local saved_back_key = saved_game and saved_game.BACK and saved_game.BACK.key
      if saved_back_key and G.P_CENTERS[saved_back_key] then
        viewed_deck = saved_back_key
      else
        for k, v in pairs(G.P_CENTERS) do
          if v and v.set == 'Back' and v.name == saved_game.BACK.name then viewed_deck = k end
        end
      end
  ...
  local ordered_names, ordered_keys, viewed_deck = {}, {}, 1
  for k, v in ipairs(G.P_CENTER_POOLS.Back) do
    ordered_names[#ordered_names+1] = v.name
    ordered_keys[#ordered_keys+1] = v.key
    if v.key == G.GAME.viewed_back.effect.center.key then viewed_deck = k end
  end
                    ...
                    {n=G.UIT.R, config={align = "cm", minh = 3.3, minw = 6.8}, nodes={
                      {n=G.UIT.C, config={align = "cm", colour = G.C.BLACK, padding = 0.15, r = 0.1, emboss = 0.05}, nodes={
                        {n=G.UIT.C, config={align = "cm"}, nodes={
                          ...
                          {n=G.UIT.C, config={align = "cm"}, nodes={
                            {n=G.UIT.O, config={id = G.GAME.viewed_back.effect.center.key, func = 'RUN_SETUP_check_back_stake_column', object = UIBox{definition = G.UIDEF.deck_stake_column(G.GAME.viewed_back.effect.center.key), config = {offset = {x=0,y=0}}}}}
                          }}  
                        }}     
                      }}}} or
                      {n=G.UIT.R, config={align = "cm", minh = 3.8}, nodes={
                        create_option_cycle({options =  ordered_names, deck_keys = ordered_keys, opt_callback = 'change_viewed_back', current_option = viewed_deck, colour = G.C.RED, w = 3.5, mid = 
...
function create_UIBox_main_menu_buttons()
  ...
  if not G.F_ENGLISH_ONLY then 
    ...
    mods_icon = Sprite(0,0,0.6,0.6,G.ASSET_ATLAS["icons"], {x=1, y=0})
    mods_icon.states.drag.can = false
  end
  local discord = nil
  local twitter = nil
  if G.F_DISCORD then 
  ...
  local t = {
    n=G.UIT.ROOT, config = {align = "cm",colour = G.C.CLEAR}, nodes={ 
      {n=G.UIT.C, config={align = "bm"}, nodes={      
        ...
        {n=G.UIT.R, config={align = "cm", padding = 0.05}, nodes={
          {n=G.UIT.T, config={text = "BALATRO(TM) is a trademark of LocalThunk. This web port is in so way endorsed or made by LocalThunk. All rights reserved.", scale = 0.24, colour = G.C.WHITE, shadow = false}}
        }},
      }},
      {n=G.UIT.C, config={align = "br", minw = 3.2, padding = 0.1}, nodes={
        ...
        not G.F_ENGLISH_ONLY and {n=G.UIT.R, config = {align = "cm", padding = 0.2, r = 0.1, emboss = 0.1, colour = G.C.L_BLACK}, nodes={
          ...
          G.IS_WEB and {n=G.UIT.R, config={align = "cm", padding = 0.15, minw = 1, r = 0.1, hover = true, colour = mix_colours(G.C.BLUE, G.C.BLACK, 0.25), button = 'open_mod_menu_web', shadow = true}, nodes={
            {n=G.UIT.O, config={object = mods_icon}},
            {n=G.UIT.T, config={text = 'Mods', scale = 0.4, colour = G.C.UI.TEXT_LIGHT, shadow = true}}
          }}
        }} or nil,
      }},
    }}
...
function G.UIDEF.web_mod_browser()
  local state = G.WEB_MOD_BROWSER_STATE or {source = 'all', status = 'Press Refresh to load mods', pending = false, items = {}}
  local local_mode = state.local_mode == true
  local local_mods = type(state.local_mods) == 'table' and state.local_mods or {}
  local stability_mode = state.stability_mode == true
  local source = tostring(state.source or 'all')
  local items = type(state.items) == 'table' and state.items or {}
  if local_mode then items = local_mods end
  local status = tostring(state.status or '')
  local per_page = tonumber(state.per_page) or 6
  local page = math.max(1, tonumber(state.page) or 1)
  local max_page = math.max(1, math.ceil((#items > 0 and #items or 1)/per_page))
  if page > max_page then page = max_page end
  local start_index = ((page - 1) * per_page) + 1
  local end_index = math.min(#items, start_index + per_page - 1)
  ...
  local function source_colour(key)
    return source == key and G.C.GREEN or mix_colours(G.C.BLUE, G.C.BLACK, 0.2)
  end

  local function web_button(label, button, colour, minw, minh, scale, ref_table)
    return {n=G.UIT.C, config={align = 'cm', padding = 0.03, r = 0.1, hover = true, colour = colour, shadow = true, button = button, one_press = true, minw = minw, minh = minh, ref_table = ref_table}, nodes={
      {n=G.UIT.T, config={text = label, scale = scale or 0.38, colour = G.C.UI.TEXT_LIGHT, shadow = true}}
    }}
  end

  local function get_preview_row_icon(item)
    if type(state._preview_icon_cache) ~= 'table' then state._preview_icon_cache = {} end

    local icon_path = tostring((item and item.icon_path) or '')
    if icon_path ~= '' then
      local cached = state._preview_icon_cache[icon_path]
      if cached == false then
        return nil
      end
      if cached and cached.object then return cached.object end

      local resolved_path = nil
      local candidates = {icon_path}
      if not string.match(icon_path, '^webbridge/') then
        candidates[#candidates + 1] = 'webbridge/' .. icon_path
      end
      for i = 1, #candidates do
        local candidate = candidates[i]
        if candidate and candidate ~= '' and love.filesystem.getInfo(candidate) then
      ...
      if not resolved_path then
        state._preview_icon_cache[icon_path] = false
        return nil
      end

      local ok_img, img = pcall(love.graphics.newImage, resolved_path)
      if ok_img and img then
        local w, h = img:getDimensions()
        if w and h and w > 0 and h > 0 then
          local atlas_name = 'web_mod_icon_' .. resolved_path:gsub('[^%w_]', '_')
          local atlas = {name = atlas_name, image = img, px = w, py = h}
          G.ASSET_ATLAS[atlas_name] = atlas
          local ok_sprite, sprite = pcall(Sprite, 0, 0, 0.55, 0.55, atlas, {x=0, y=0})
          if ok_sprite and sprite then
            sprite.states.drag.can = false
            state._preview_icon_cache[icon_path] = {object = sprite}
            return sprite
          end
        end
      end
    ...
    if state._preview_icon_fallback then return state._preview_icon_fallback end
    local fallback = Sprite(0,0,0.55,0.55,G.ASSET_ATLAS["icons"], {x=3, y=0})
    fallback.states.drag.can = false
    state._preview_icon_fallback = fallback
    return fallback
  end

  local rows = {
    {n=G.UIT.R, config={align = 'cm', padding = 0.05}, nodes={
      {n=G.UIT.T, config={text = 'Mod Manager', scale = 0.56, colour = G.C.UI.TEXT_LIGHT, shadow = true}}
    }},
    {n=G.UIT.R, config={align = 'cm', padding = 0.04}, nodes={
      {n=G.UIT.C, config={align = 'cm', padding = 0.02}, nodes={
        web_button(local_mode and 'Reload List' or 'Refresh', 'refresh_mod_catalog_web', G.C.ORANGE, 1.8, 0.7, 0.4)
      }}
    }},
    {n=G.UIT.R, config={align = 'cm', padding = 0.04, minw = 8.8, r = 0.08, colour = G.C.L_BLACK}, nodes={
      {n=G.UIT.T, config={text = status ~= '' and status or 'Ready', scale = 0.35, colour = state.pending and G.C.ORANGE or G.C.UI.TEXT_LIGHT, shadow = true}}
    }}
  }
  ...
  if local_mode then
    if #items == 0 then
      list_rows[#list_rows + 1] = {n=G.UIT.R, config={align = 'cm', padding = 0.12}, nodes={
        {n=G.UIT.T, config={text = 'No local mods configured', scale = 0.34, colour = G.C.WHITE, shadow = true}}
      }}
    else
      for i = start_index, end_index do
        local item = items[i] or {}
        local enabled = item.enabled == true
        local implemented = item.implemented ~= false
        local title = truncate_text(item.title or 'Unnamed Mod', 42)
        local byline = truncate_text('Source: ' .. tostring(item.source or 'Unknown'), 54)
        local row_colour = implemented and mix_colours(G.C.BLACK, G.C.L_BLACK, 0.35) or mix_colours(G.C.GREY, G.C.BLACK, 0.65)
        local title_colour = implemented and G.C.UI.TEXT_LIGHT or G.C.UI.TEXT_INACTIVE
        local byline_colour = implemented and G.C.WHITE or mix_colours(G.C.WHITE, G.C.GREY, 0.65)
        local button_colour = implemented and (enabled and G.C.RED or G.C.GREEN) or G.C.GREY
        local button_label = implemented and (enabled and 'Disable' or 'Enable') or 'Not Impl'
        local button_action = implemented and 'toggle_local_mod_web' or 'nil'

        list_rows[#list_rows + 1] = {n=G.UIT.R, config={align = 'cm', padding = 0.045, minw = 8.8, r = 0.08, colour = row_colour}, nodes={
          {n=G.UIT.C, config={align = 'cl', minw = 6.9, maxw = 6.9, padding = 0.05}, nodes={
            {n=G.UIT.R, config={align = 'cl', minw = 6.8, maxw = 6.8}, nodes={{n=G.UIT.T, config={text = title, scale = 0.38, colour = title_colour, shadow = true}}}},
            {n=G.UIT.R, config={align = 'cl', minw = 6.8, maxw = 6.8}, nodes={{n=G.UIT.T, config={text = byline, scale = 0.29, colour = byline_colour, shadow = true}}}},
          }},
          {n=G.UIT.C, config={align = 'cm', minw = 1.9, maxw = 1.9, padding = 0.03}, nodes={
            UIBox_button{button = button_action, ref_table = {mod_id = item.id}, colour = button_colour, minw = 1.7, minh = 0.68, label = {button_label}, scale = 0.33, text_colour = implemented and G.C.UI.TEXT_LIGHT or G.C.UI.TEXT_INACTIVE}
          }}
        }}
      end
      list_rows[#list_rows + 1] = {n=G.UIT.R, config={align = 'cm', padding = 0.08}, nodes={
        {n=G.UIT.T, config={text = 'Toggle changes apply to local manager state. Restart required for full runtime unload/load.', scale = 0.27, colour = mix_colours(G.C.WHITE, G.C.GREY, 0.45), shadow = true}}
      }}
    end
  elseif stability_mode then
    local stable_count = tonumber(state.catalog_count) or #items
    local preview_items = type(state.preview_items) == 'table' and state.preview_items or {}
    list_rows[#list_rows + 1] = {n=G.UIT.R, config={align = 'cm', padding = 0.16}, nodes={
      {n=G.UIT.T, config={text = state.pending and 'Loading catalog in background…' or ('Catalog ready: ' .. tostring(stable_count) .. ' mods'), scale = 0.4, colour = G.C.WHITE, shadow = true}}
    }}
    if #preview_items > 0 then
      local p = preview_items[1] or {}
      local title = truncate_text(p.title or 'Unnamed Mod', 34)
      local byline = truncate_text('by ' .. tostring(p.author or 'Unknown') .. ' • ' .. tostring(p.source or 'Web') .. ' • ' .. tostring(p.downloads or 0), 54)
      local desc = truncate_text(p.description or '', 72)
      local row_icon = get_preview_row_icon(p) or state._preview_icon_fallback
      if not row_icon then
        local fallback = Sprite(0,0,0.55,0.55,G.ASSET_ATLAS["icons"], {x=3, y=0})
        fallback.states.drag.can = false
        state._preview_icon_fallback = fallback
        row_icon = fallback
      ...
      list_rows[#list_rows + 1] = {n=G.UIT.R, config={align = 'cm', padding = 0.045, minw = 8.8, r = 0.08, colour = mix_colours(G.C.BLACK, G.C.L_BLACK, 0.35)}, nodes={
        {n=G.UIT.C, config={align = 'cm', minw = 0.7, maxw = 0.7, padding = 0.03}, nodes={{n=G.UIT.O, config={object = row_icon}}}},
        {n=G.UIT.C, config={align = 'cl', minw = 6.0, maxw = 6.0, padding = 0.05}, nodes={
          {n=G.UIT.R, config={align = 'cl', minw = 5.9, maxw = 5.9}, nodes={{n=G.UIT.T, config={text = title, scale = 0.4, colour = G.C.UI.TEXT_LIGHT, shadow = true}}}},
          {n=G.UIT.R, config={align = 'cl', minw = 5.9, maxw = 5.9}, nodes={{n=G.UIT.T, config={text = byline, scale = 0.3, colour = G.C.WHITE, shadow = true}}}},
          {n=G.UIT.R, config={align = 'cl', minw = 5.9, maxw = 5.9}, nodes={{n=G.UIT.T, config={text = desc ~= '' and desc or 'No description', scale = 0.29, colour = mix_colours(G.C.WHITE, G.C.GREY, 0.55), shadow = true}}}},
        }}
        ,{n=G.UIT.C, config={align = 'cm', minw = 1.7, maxw = 1.7, padding = 0.03}, nodes={
          UIBox_button{button = 'open_mod_entry_url', ref_table = {mod_url = p.url, download_url = p.download_url, title = p.title}, colour = G.C.BLUE, minw = 1.5, minh = 0.7, label = {(p.download_url and p.download_url ~= '') and 'Install' or 'Open'}, scale = 0.34}
        }}
      }}
    else
      list_rows[#list_rows + 1] = {n=G.UIT.R, config={align = 'cm', padding = 0.12}, nodes={
        {n=G.UIT.T, config={text = 'Waiting for first mod row…', scale = 0.3, colour = mix_colours(G.C.WHITE, G.C.GREY, 0.45), shadow = true}}
      }}
    end
  elseif #items == 0 then
    list_rows[#list_rows + 1] = {n=G.UIT.R, config={align = 'cm', padding = 0.2}, nodes={
      {n=G.UIT.T, config={text = state.pending and 'Loading…' or 'No mods loaded yet', scale = 0.42, colour = G.C.WHITE, shadow = true}}
    }}
  else
    for i = start_index, end_index do
      local item = items[i]
      local title = truncate_text(item.title or 'Unnamed Mod', 34)
      local byline = truncate_text('by ' .. tostring(item.author or 'Unknown') .. ' • ' .. tostring(item.source or 'Web') .. ' • ' .. tostring(item.downloads or 0), 54)
      local desc = truncate_text(item.description or '', 72)
      local row_icon = nil

      list_rows[#list_rows + 1] = {n=G.UIT.R, config={align = 'cm', padding = 0.045, minw = 8.8, r = 0.08, colour = mix_colours(G.C.BLACK, G.C.L_BLACK, 0.35)}, nodes={
        {n=G.UIT.C, config={align = 'cm', minw = 0.7, maxw = 0.7, padding = 0.03}, nodes={
          row_icon and {n=G.UIT.O, config={object = row_icon}} or {n=G.UIT.T, config={text = '', scale = 0.1, colour = G.C.WHITE}},
        }},
        {n=G.UIT.C, config={align = 'cl', minw = 6.0, maxw = 6.0, padding = 0.05}, nodes={
          {n=G.UIT.R, config={align = 'cl', minw = 5.9, maxw = 5.9}, nodes={{n=G.UIT.T, config={text = title, scale = 0.4, colour = G.C.UI.TEXT_LIGHT, shadow = true}}}},
          {n=G.UIT.R, config={align = 'cl', minw = 5.9, maxw = 5.9}, nodes={{n=G.UIT.T, config={text = byline, scale = 0.3, colour = G.C.WHITE, shadow = true}}}},
        ...
        {n=G.UIT.C, config={align = 'cm', minw = 1.7, maxw = 1.7, padding = 0.03}, nodes={
          UIBox_button{button = 'open_mod_entry_url', ref_table = {mod_url = item.url, download_url = item.download_url, title = item.title}, colour = G.C.BLUE, minw = 1.5, minh = 0.7, label = {item.download_url and item.download_url ~= '' and 'Install' or 'Open'}, scale = 0.34}
        }}
      }}
    end
  end

  rows[#rows + 1] = {n=G.UIT.R, config={align = 'cm', minw = 8.95, minh = 5.0, maxh = 5.0, r = 0.08, colour = G.C.BLACK, padding = 0.08}, nodes={
    {n=G.UIT.C, config={align = 'tm', minw = 8.75, minh = 4.8, maxh = 4.8, padding = 0.03}, nodes=list_rows}
  }}

  rows[#rows + 1] = {n=G.UIT.R, config={align = 'cm', padding = 0.04}, nodes={
    {n=G.UIT.C, config={align = 'cm', padding = 0.02}, nodes={
      web_button('Prev', 'mod_catalog_prev_page', mix_colours(G.C.BLUE, G.C.BLACK, 0.15), 1.5, 0.62, 0.36),
      {n=G.UIT.C, config={align = 'cm', minw = 2.2, minh = 0.62, padding = 0.05}, nodes={{n=G.UIT.T, config={text = tostring(page) .. '/' .. tostring(max_page), scale = 0.34, colour = G.C.UI.TEXT_LIGHT, shadow = true}}}},
      web_button('Next', 'mod_catalog_next_page', mix_colours(G.C.BLUE, G.C.BLACK, 0.15), 1.5, 0.62, 0.36)
    }}
  }}
